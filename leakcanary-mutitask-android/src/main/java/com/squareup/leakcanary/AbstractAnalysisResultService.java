...
public abstract class AbstractAnalysisResultService extends IntentService {
  ...
  private static final String REMAINDER_QUEUE_SIZE="remainder_queue_size";

  public static void sendResultToListener(Context context, String listenerServiceClassName,
      HeapDump heapDump, AnalysisResult result,int queueSize) {
    ...
    intent.putExtra(REMAINDER_QUEUE_SIZE,queueSize);
  ...
  @Override protected final void onHandleIntent(Intent intent) {
    ...
    int remainderSize = intent.getIntExtra(REMAINDER_QUEUE_SIZE,0);
    try {
      onHeapAnalyzed(heapDump, result,remainderSize);
    } finally {
   ...
   */
  protected abstract void onHeapAnalyzed(HeapDump heapDump, AnalysisResult result,int remainderSize);
}
