...
public class DisplayLeakService extends AbstractAnalysisResultService {
  ...
  @Override protected final void onHeapAnalyzed(HeapDump heapDump, AnalysisResult result, final int
          remainderSize) {
    ...
    CanaryLog.d("leakInfo:"+leakInfo+" shouldSaveResult:"+shouldSaveResult);
    if (shouldSaveResult) {
      heapDump = renameHeapdump(heapDump);
      resultSaved = saveResult(heapDump, result);
      CanaryLog.d("save result is "+resultSaved);
    }
    ...
    if(remainderSize > 0) {
      contentText += " ,analyzing " + remainderSize + " object";
    }
    ...
    handler.post(new Runnable() {
      ...
      public void run() {
        if(remainderSize>0){
          Toast.makeText(getApplicationContext(),contentTitle+" analyzing " + remainderSize +
                  "object",
                  Toast.LENGTH_SHORT).show();
        }else{
          Toast.makeText(getApplicationContext(),contentTitle,Toast.LENGTH_SHORT).show();
        }
      }
    });
