...
public final class AndroidHeapDumper implements HeapDumper {
  ...
  private static final String HEAPDUMP_FILE = "suspected_leak_heapdump";
  private static final String SUFFIX_FILE = ".hprof";
...
//
 //  Toast toast = waitingForToast.get();
    try {
      CanaryLog.d("start dump heap ....");
      Debug.dumpHprofData(heapDumpFile.getAbsolutePath());
      CanaryLog.d("heapDumpFile path:"+heapDumpFile.getAbsolutePath());
      //cancelToast(toast);
      return heapDumpFile;
    } catch (Exception e) {
      cleanup();
  ...
  File getHeapDumpFile() {
    return new File(leakDirectoryProvider.leakDirectory(), HEAPDUMP_FILE+"_"+UUID.randomUUID()
            .toString()+SUFFIX_FILE);
  }
